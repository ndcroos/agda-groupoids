...
module Arrow where
  ...
  module _ where
    ...
    dom (lift (cod-fib← A) {b}{e = ((σ , τ) T.▸ e)} f) =
      (σ , b) T.▸
      (cmp₀ A f e)
    map (lift (cod-fib← A) f) =
      (idn₀ A , f) T.▸
      (⊢idn₀-λ A)
    lhs (edge (car (lift (cod-fib← A) f)) (horn img dia coh)) =
      ...
      (inv₁ A
        (seq₁ A
          ...
          (seq₁ A
            (seq₀*-ρ A coh)
            (inv₁ A (T.Σ.snd dia)))))
    coh-seq (edge (car (lift (cod-fib← A) f)) 𝔥) =
      ι (inv₁ A (⊢idn₀-λ A) , inv₁ A (Horn.coh 𝔥))
    coh-img (edge (car (lift (cod-fib← A) f)) 𝔥) =
      idn₁ A
    unique (edge (car (lift (cod-fib← A) f)) 𝔥) #lhs #seq #img =
      ι (inv₁ A (seq₁ A (T.⊗.fst (T.⊔⇑.π #seq)) (⊢idn₀-λ A)) , #img)
    coe (lift (cod-fib← A) f) =
      ≅.idn Op[ A ]
    coh (lift (cod-fib← A) f) =
      ⊢idn₀-ρ A
